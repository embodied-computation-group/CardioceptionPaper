...
# Model -------------------------------------------------------------------
#nR_S1_tot = list(list(c(23.,  9.,  7.,  5.,  3.,  3.,  0.,  0.)))
#nR_S2_tot = list(list(c(0.,  1.,  4.,  6.,  9.,  6., 10., 14.)))
metad_1wayANOVA <- function (nR_S1_tot, nR_S2_tot) {
  # nR_S1_tot and nR_S2_tot should be lists of size N subjects.
  # Each list contain 4 responses vectors.
  # Design matrix: Condition 1: (1, 0)
  ...
  aov_model <- jags.model("Bayes_metad_1wayANOVA.txt", data = data,
  ...
  output <- coda.samples( 
    model          = aov_model,
    variable.names = c("muBd_Condition1", "lamBd_Condition1", "sigD_Condition1", "Mratio"),
    n.iter         = 10000,
    thin           = 1 )
